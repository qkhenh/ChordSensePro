...
def aggregate_attempts(**context) -> None:
    ...
    Pushes accuracies list to XCom for next task.
    Uses list_by_date() — SQL WHERE filter, no full table scan.
    """
    today = date.today()

    async def _run():
        async with get_session() as session:
            repo = ChordAttemptRepository(session)
            all_attempts = await repo.list_by_date(today)   # SQL WHERE — no full scan
...
def rolling_accuracy(**context) -> None:
    ...
    async def _run():
        async with get_session() as session:
            mastery_repo = ChordMasteryRepository(session)
            rolled = []

            # Batch-fetch mastery for all unique users — avoids N+1 queries
            unique_users = {row["user_id"] for row in accuracies}
            user_history: dict[str, list] = {}
            for uid in unique_users:
                user_history[uid] = await mastery_repo.get_by_user(uid)
    ...
    result = asyncio.run(_run())
    ti.xcom_push(key="rolled", value=result)
...
def update_mastery(**context) -> None:
    ...
    async def _run():
        async with get_session() as session:
            repo = ChordMasteryRepository(session)
            ...
            for row in rolled:
                ...
                entity.recompute_mastery()   # sets is_mastered if >= 80%
